...
@Component({
  selector: 'page-information-finder-counseling-centers',
  templateUrl: 'information-finder-counseling-centers.html'
  // styleUrls: ['/src/pages/information-finder/information-finder.scss']
})
export class InformationFinderCounselingCentersPage {
  ...
  OnDeleteItem(id){
    if(confirm("Are you sure you want to delete this record?")) {
      ...
      this.loader.present();
      this.http.delete(this.apiEndpoint + "counseling-center/delete-counseling-center?id=" + id, this.requestOptions)
      .map(res =>  res.json()).subscribe(res =>{
        this.loader.dismiss();
        if(res.success) {
  ...
  EditCounselingCenter(form,recordId,currentImagePath) {
    ...
    let counselingCenter = {
      _id : recordId,
      "name" : form.value.name,
      "classification" : form.value.classification,
      "assignment" : form.value.assignment,
    ...
    if(editInputFile.files[0]){
      imagesFormData.append("counseling-center-images", editInputFile.files[0], editInputFile.files[0]["name"]);
    } 
    //2)Upload Images
    this.fileSvc.httpUploadFiles(imagesFormData,"counseling-center-images").subscribe(resImage=>{
      if (!resImage.success) {
        ...
        //Add Priest Record
        counselingCenter.image= JSON.stringify(imagesArr);
        //-------------------------------------------Priest Record Upload Emd -------------------------------------------

        // Proceed to adding Counseling Center Record
        this.http.put(this.apiEndpoint + "counseling-center/update-counseling-center?id=" + recordId,counselingCenter, this.requestOptions)
        .map(res =>  res.json()).subscribe(res =>{
          this.showLoading = false;
          this.loader.dismiss();
          if(res.success) {
            this.showSuccess = true;
            this.successMessage = "Church Person has been updated successfully."
            for(let indx = 0;indx < this.searchResults.length;indx++) {
              if(this.searchResults[indx]._id === recordId) {
                counselingCenter.image = JSON.parse(counselingCenter.image);
                this.searchResults[indx] = counselingCenter;
              }
            }
  ...
  AddNewCounselingCenter(form){
    ...
    let imagesFormData = new FormData();
    if(addInputFile.files[0]){
      imagesFormData.append("counseling-center-images", addInputFile.files[0], addInputFile.files[0]["name"]);
    } 
    //2)Upload Images
    this.fileSvc.httpUploadFiles(imagesFormData,"counseling-center-images").subscribe(resImage=>{
      if (!resImage.success) {
        ...
        // Proceed to adding Counseling Center Record
        this.http.post(this.apiEndpoint + "counseling-center/add-counseling-center",counselingCenter, this.requestOptions)
        .map(res =>  res.json()).subscribe(res =>{
          this.showLoading = false;
          this.loader.dismiss();
          if(res.success) {
            this.showSuccess = true;
            this.successMessage = "Church Person has been added successfully."
            setTimeout(()=>{
              this.showSuccess = false;
              this.CloseAddModal();
            },1500);
