...
@Component({
  selector: 'page-information-finder-catholic-school',
  templateUrl: 'information-finder-catholic-school.html'
  // styleUrls: ['/src/pages/information-finder/information-finder.scss']
})
export class InformationFinderCatholicSchoolPage {
    ...
    ShowAddModal(){
      ...
      OnDeleteItem(id){
        if(confirm("Are you sure you want to delete this record?")) {
            ...
            this.loader.present();
            this.http.delete(this.apiEndpoint + "catholic-school/delete-catholic-school?id=" + id, this.requestOptions)
            .map(res =>  res.json()).subscribe(res =>{
                this.loader.dismiss();
                if(res.success) {
      ...
      EditCatholicSchool(form,recordId,currentImagePath) {
        ...
        let catholicSchool = {
          ...
          "management" : form.value.management,
          "address" : form.value.address,
          "directorDirectressPrincipal" : form.value.directorDirectressPrincipal,
        ...
        if(editInputFile.files[0]){
          imagesFormData.append("catholic-school-images", editInputFile.files[0], editInputFile.files[0]["name"]);
        } 
        //2)Upload Images
        this.fileSvc.httpUploadFiles(imagesFormData,"catholic-school-images").subscribe(resImage=>{
          if (!resImage.success) {
            ...
            //Add Priest Record
            catholicSchool.image= JSON.stringify(imagesArr);
            //-------------------------------------------Priest Record Upload Emd -------------------------------------------
    
            // Proceed to adding Catholic School Record
            this.http.put(this.apiEndpoint + "catholic-school/update-catholic-school?id=" + recordId,catholicSchool, this.requestOptions)
            .map(res =>  res.json()).subscribe(res =>{
              this.showLoading = false;
              this.loader.dismiss();
              if(res.success) {
                this.showSuccess = true;
                this.successMessage = "Catholic School has been updated successfully."
                for(let indx = 0;indx < this.searchResults.length;indx++) {
                  if(this.searchResults[indx]._id === recordId) {
                    catholicSchool.image = JSON.parse(catholicSchool.image);
                    this.searchResults[indx] = catholicSchool;
                  }
                }
      ...
      AddNewCatholicSchool(form){
        ...
        let imagesFormData = new FormData();
        if(addInputFile.files[0]){
          imagesFormData.append("catholic-school-images", addInputFile.files[0], addInputFile.files[0]["name"]);
        } 
        //2)Upload Images
        this.fileSvc.httpUploadFiles(imagesFormData,"catholic-school-images").subscribe(resImage=>{
          if (!resImage.success) {
            ...
            // Proceed to adding Catholic School Record
            this.http.post(this.apiEndpoint + "catholic-school/add-catholic-school",catholicSchool, this.requestOptions)
            .map(res =>  res.json()).subscribe(res =>{
              this.showLoading = false;
              this.loader.dismiss();
              if(res.success) {
                this.showSuccess = true;
                this.successMessage = "Catholic School has been added successfully."
                setTimeout(()=>{
                  this.showSuccess = false;
                  this.CloseAddModal();
                },1500);
