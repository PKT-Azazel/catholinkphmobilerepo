...
@Component({
  selector: 'page-information-finder-retreat-centers',
  templateUrl: 'information-finder-retreat-centers.html'
  // styleUrls: ['/src/pages/information-finder/information-finder.scss']
})
export class InformationFinderRetreatCentersPage {
  ...
  OnDeleteItem(id){
    ...
    this.loader.present();
      this.http.delete(this.apiEndpoint + "retreat-center/delete-retreat-center?id=" + id, this.requestOptions)
      .map(res =>  res.json()).subscribe(res =>{
        this.loader.dismiss();
        if(res.success) {
  ...
  EditRetreatCenter(form,recordId,currentImagePath) {
  ...
  this.loader.present();
    ...
    if(editInputFile.files[0]){
      imagesFormData.append("retreat-center-images", editInputFile.files[0], editInputFile.files[0]["name"]);
    } 
    //2)Upload Images
    this.fileSvc.httpUploadFiles(imagesFormData,"retreat-center-images").subscribe(resImage=>{
      if (!resImage.success) {
        ...
        //Add Priest Record
        retreatCenter.image= JSON.stringify(imagesArr);
        //-------------------------------------------Priest Record Upload Emd -------------------------------------------

        // Proceed to adding Retreat Center Record
        this.http.put(this.apiEndpoint + "retreat-center/update-retreat-center?id=" + recordId,retreatCenter, this.requestOptions)
        .map(res =>  res.json()).subscribe(res =>{
          this.showLoading = false;
          this.loader.dismiss();
          if(res.success) {
            this.showSuccess = true;
            this.successMessage = "Retreat Center has been updated successfully."
            for(let indx = 0;indx < this.searchResults.length;indx++) {
              if(this.searchResults[indx]._id === recordId) {
                retreatCenter.image = JSON.parse(retreatCenter.image);
                this.searchResults[indx] = retreatCenter;
              }
            }
  ...
  this.loader.present();
    ...
    let imagesFormData = new FormData();
    if(addInputFile.files[0]){
      imagesFormData.append("retreat-center-images", addInputFile.files[0], addInputFile.files[0]["name"]);
    } 
    //2)Upload Images
    this.fileSvc.httpUploadFiles(imagesFormData,"retreat-center-images").subscribe(resImage=>{
      if (!resImage.success) {
        ...
        // Proceed to adding Retreat Center Record
        this.http.post(this.apiEndpoint + "retreat-center/add-retreat-center",retreatCenter, this.requestOptions)
        .map(res =>  res.json()).subscribe(res =>{
          this.showLoading = false;
          this.loader.dismiss();
          if(res.success) {
            this.showSuccess = true;
            this.successMessage = "Retreat Center has been added successfully."
            setTimeout(()=>{
              this.showSuccess = false;
              this.CloseAddModal();
            },1500);
